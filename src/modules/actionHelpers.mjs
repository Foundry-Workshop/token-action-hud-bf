...
export function awardXP(actors) {
  new Dialog({
    title: game.i18n.localize("tokenActionHud.black-flag.actions.awardXP.DialogTitle"),
    content: `<form style="padding: 5px 0">
              <div class="form-group">
                <label>${game.i18n.localize("tokenActionHud.black-flag.actions.awardXP.Amount")}</label> 
                <input type="text" id="xp-amount" value="50" />
              </div>
              <div class="form-group">
                <label>${game.i18n.localize("tokenActionHud.black-flag.actions.awardXP.Reason")}</label> 
                <input type="text" id="reason" value="" placeholder="${game.i18n.localize("tokenActionHud.black-flag.actions.awardXP.Reason").toLowerCase()}" />
              </div>
          </form>`,
    buttons: {
      yes: {
        icon: "<i class='fas fa-check'></i>",
        label: game.i18n.localize("tokenActionHud.black-flag.actions.awardXP.Award"),
        callback: (html) => {
          const xp = Math.round(html.find("#xp-amount").val());
          const reason = html.find("#reason").val();

          if (isNaN(xp))
            return Utility.notify(game.i18n.localize("tokenActionHud.black-flag.actions.awardXP.XPNaN"), {type: "warning"});
          if (!reason)
            return Utility.notify(game.i18n.localize("tokenActionHud.black-flag.actions.awardXP.ReasonEmpty"), {type: "warning"});
